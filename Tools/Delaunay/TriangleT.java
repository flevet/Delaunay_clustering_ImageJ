...
public class TriangleT {
    ...
    public PointT getPoint(int _index){
        switch(_index){
            case 0:  return m_a;
            case 1:  return m_b;
            case 2:  return m_c;
    ...
    public double getArea(){
        /*double d1 = Point2D.Double.distance( m_a.x, m_a.y, m_b.x, m_b.y ), d2 = Point2D.Double.distance( m_b.x, m_b.y, m_c.x, m_c.y ), d3 = Point2D.Double.distance( m_c.x, m_c.y, m_a.x, m_a.y );
        double perimeter = ( d1 + d2 + d3 ) / 2.;
        return Math.sqrt( perimeter * ( perimeter - d1 ) * ( perimeter - d2 ) * ( perimeter - d3 ) );*/
        /*double area = ( m_a.x * ( m_b.y - m_c.y ) ) + ( m_b.x * ( m_c.y - m_a.y ) ) + ( m_c.x * ( m_a.y - m_b.y ) );
