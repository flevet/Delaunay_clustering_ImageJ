...
public class EdgeT implements EdgeGraph{
    ...
    public void recomputeSkeleton(){
        skel.clear();
        /*if( m_tri.isEmpty() ){
            skel.add( v1.getTriangle().getBarycenter() );
        }
        else{
            TriangleT next = m_tri.get( 0 ), cur = null;
            if( next.isTerminal() )
                skel.add( next.getBarycenter() );
            else{
                if( !v1.getTriangle().isTrimmed() ){
                    cur = v1.getTriangle();
                    skel.add( next.getBarycenter() );
                }
                else{
                    ArrayList < TriangleT > neighs = next.getNeighbors();
                    for( int n = 0; n < neighs.size() && cur == null; n++ )
                        if( neighs.get( n ).isTrimmed() )
                            cur = neighs.get( n );
                    if( cur == null ){
                        ...
                        skel.add( next.getCenterSharedEdge( cur ) );
                }
            }
            cur = next;
            for( int i = 1; i < m_tri.size(); i++ ){
                next = m_tri.get( i );
                skel.add( next.getCenterSharedEdge( cur ) );
                cur = next;
            }
            if( !v2.getTriangle().isTrimmed() )
                skel.add( v2.getTriangle().getBarycenter() );
        }*/
        /*for( int i = 0; i < m_tri.size(); i++ )
            skel.add( m_tri.get( i ).getBarycenter() );*/
    ...
    public boolean isEdgeTrimmed(){
        ...
        return test;
        /*if( test ){
            if( !v1.getTriangle().isNeighbor( v2.getTriangle() ) )
                return ( v1.degree() == 3 && v2.degree() == 3 );
            return true;
        }
        return false;*/
    }
    ...
    public TriangleT getNeighborTriangleFromTriangleSeed( SeedT _s ){
        ...
        if( m_tri.isEmpty() ){
            ...
            for(TriangleT tmp : m_tri)
                if(t.isNeighbor(tmp))
                    return tmp;
            /*if( t.isNeighbor( m_tri.get( 0 ) ) )
                return m_tri.get( 0 );
            if( t.isNeighbor( m_tri.get( m_tri.size() - 1 ) ) )
                return m_tri.get( m_tri.size() - 1 );*/
        }
        return null;
    }
    ...
    public String toString(){
        return "e{" + v1.toString() + ", " + v2.toString() + ", id=" + id + ", done=" + m_marked + "}";
    }
